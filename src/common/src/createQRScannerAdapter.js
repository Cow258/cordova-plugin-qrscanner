      ...
      if (isIOS) {
        ...
        switch (true) {
          case (Array.isArray(param)):
            fStyle = FeedbackStyle.rigid; break
          case (typeof param === 'string'):
            fStyle = param; break
          case (param === 1):
            fStyle = FeedbackStyle.soft; break
          case (param < 100):
            fStyle = FeedbackStyle.light; break
          case (param >= 100 && param < 300):
            fStyle = FeedbackStyle.medium; break
          case (param >= 300):
            fStyle = FeedbackStyle.heavy; break
        }
        if (fStyle === null) return
        return cordova.exec(null, null, 'QRScanner', 'vibrate', [fStyle])
      }

      if ((typeof param === 'number') && param !== 0) {
        return cordova.exec(null, null, 'Vibration', 'vibrate', [param])
      }
      // vibrate with array ( i.e. vibrate([3000]) )
      if ((typeof param === 'object') && param.length === 1) {
        // cancel if vibrate([0])
        if (param[0] === 0) {
          return exec(null, null, 'Vibration', 'cancelVibration', [])
        }
        return exec(null, null, 'Vibration', 'vibrate', [param[0]])
      }
      // vibrate with a pattern
      if ((typeof param === 'object') && param.length > 1) {
        var repeat = -1 // no repeat
        return exec(null, null, 'Vibration', 'vibrateWithPattern', [param, repeat])
      }
